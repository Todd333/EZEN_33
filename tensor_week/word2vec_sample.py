...
rc('font', family = font_manager
   .FontProperties(fname = 'C:\Windows\Fonts\나눔고딕.TTF')
   .get_name())
...
#리스트에서 문자열 인덱스로 뽑아서 사용하기 위해
#이를 표현하기 위한 연관배열과
#단어 리스트에서 단어를 참조할 수 있는 인덱스 배열을 만든다
    ...
    for step in range(1, training_epoch + 1):
        ...
        _, loss_val = sess.run([train_op, loss],
                                {inputs: batch_inputs,
                                 labels: batch_labels})
        if step % 10 == 0:
            print("loss at stop", step, ":", loss_val)
    trained_embeddings = embeddings.eval()

for i, label in enumerate(word_list):
    x, y = trained_embeddings[i]
    plt.scatter(x, y)
    plt.annotate(label, xy=(x, y), xytest=(5, 2),

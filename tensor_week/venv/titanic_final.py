...
#SVC()에서 사용할 테스트 아이디 작성... 검증 이후에 제출할 테스트 값으로 사용함
...
k_fol = KFold(n_splits=10, shuffle= True, random_state=0)
print('KNN 방식 검증')
...
scoring = 'accuracy'
score = corss_val_score(clf, train_data, target, cv=k_fold,
                        n_jobs= 1, scoring= scoring)
print(round(np.mean(score)*100, 2))
...
#SVM 방식이 가장 정확도가 높으므로 이 방식으로 파일을 제작하여 제출하기로 함

clf = SVC()
clf.fi (train_data, target)
